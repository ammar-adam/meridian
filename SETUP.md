...
(Mondays preferred. Vercel Hobby allows **at most 2 crons** in `vercel.json`, each **once/day** — currently:
`/api/cron/school-coverage` 04:00 UTC + `/api/cron/ingest` 06:30 UTC.
Higher cadence lives in GitHub Actions: `school-coverage.yml` (every 6h), `prod-corpus-pump.yml`, `bulk-corpus.yml`, `index-check.yml`, `ingest.yml`.)
...
| Job | Cadence | Auth |
|---|---|---|
| `/api/cron/school-coverage` | Vercel daily 04:00 UTC + GHA every 6h | `CRON_SECRET` |
| `/api/cron/ingest` | Vercel daily 06:30 UTC | `CRON_SECRET` |
