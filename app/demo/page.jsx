    ...
    <div className="flex flex-wrap items-start justify-between gap-3 border-b py-3 last:border-0" style={{ borderColor: 'var(--m-border)' }}>
      <div className="min-w-0 flex-1">
        <div className="flex items-center gap-2">
          <span
            className={`inline-block h-2 w-2 shrink-0 rounded-full ${ok ? 'bg-[color:var(--m-accent)]' : ok === false ? 'bg-amber-400' : 'bg-[color:var(--m-muted-2)]'}`}
            aria-hidden
          />
  ...
  return (
    <li className="flex gap-3 text-[14px]">
      ...
      <div>
        ...
        {href && (
          <Link href={href} className="mt-1 inline-block text-[13px] font-medium text-[color:var(--m-accent)] hover:underline">
            {hrefLabel || 'Open →'}
          </Link>
        )}
      </div>
  ...
  return (
    ...
    >
      <WorkspacePage width="medium">
        <div className={`mb-6 rounded-xl border px-4 py-3 ${demoReady ? 'border-[color:var(--m-accent-line)] bg-[color:var(--m-accent-soft)]' : 'border-amber-400/30 bg-amber-400/10'}`}>
          <p className={`text-[13px] font-semibold ${demoReady ? 'text-[color:var(--m-accent)]' : 'text-amber-200'}`}>
        ...
        <WorkspaceSection title="Env vars (minimum)" description="Paste on Vercel → Settings → Environment Variables → redeploy.">
          <pre className="overflow-x-auto rounded-lg border border-[color:var(--m-border)] bg-black/60 p-4 font-mono text-[12px] leading-relaxed text-[color:var(--m-accent)]">
...
CRON_SECRET=...             # operator curls only`}
          ...
          <p className="mt-3 text-[13px]" style={{ color: 'var(--m-muted)' }}>
            Copy template: <code className="rounded bg-white/10 px-1 text-[color:var(--m-muted)]">.env.demo</code> in repo root.
            Terminal: <code className="rounded bg-white/10 px-1 text-[color:var(--m-muted)]">npm run debate</code> (must avg ≥ 7) ·{' '}
            <code className="rounded bg-white/10 px-1 text-[color:var(--m-muted)]">./scripts/demo-preflight.sh</code>
          </p>
        </WorkspaceSection>
        ...
        <WorkspaceSection title="Reset browser state (optional)" description="If a prior demo left the wrong fund or empty Flow.">
          <p className="text-[13px]" style={{ color: 'var(--m-muted)' }}>
            DevTools → Application → Local Storage → clear <code className="rounded bg-white/10 px-1 text-[color:var(--m-muted)]">meridian_funds_store</code>
