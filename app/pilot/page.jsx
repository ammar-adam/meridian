      ...
      <WorkspacePage width="medium">
        ...
        {(feedStats || bulkFill?.ran) && (
          <div className="mb-6 rounded-xl border border-[color:var(--m-accent-line)] bg-[color:var(--m-accent-soft)] px-4 py-3">
            <p className="text-[13px] font-semibold text-[color:var(--m-accent)]">Live feed parity</p>
            {feedStats && (
              <p className="mt-1 text-[13px] text-[color:var(--m-muted)]">
                {flowFeedStatsLine(feedStats)}
                {benchmark?.feedParity?.mandate ? ` · mandate: ${benchmark.feedParity.mandate}` : ''}
              </p>
            )}
            {bulkFill?.ran && (
              <p className="mt-1 font-mono text-[12px] text-[color:var(--m-accent)]">
          ...
          <ol className="space-y-3">
            {study.loop.map((step, i) => (
              <li key={step.step} className="flex gap-3 text-[14px]">
                <span className="font-mono text-[11px] text-[color:var(--m-accent)]">{String(i + 1).padStart(2, '0')}</span>
                <div>
              ...
              <tbody>
                {study.proofCompanies.map(c => (
                  <tr key={c.name}>
                    ...
                    <td className="text-[12px] text-[color:var(--m-accent)]">{c.coverage}</td>
                    <td>
                      {c.linkedin ? (
