...
/** True when production Clerk live keys are configured (not pk_test). */
export function isClerkLive() {
  const pk = clerkPublishableKey()
  return Boolean(pk?.startsWith('pk_live_') && clerkSecretKey()?.startsWith('sk_live_'))
}
