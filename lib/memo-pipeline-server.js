...
import { ingestBriefCompany } from '@/lib/server/brief-ingest'
...
export async function runBriefPipeline({
  ...
  try {
    ...
    // Compound: research cache + durable company/sighting for the graph.
    if (domain && researchPasses?.length && !fromRecord) {
      saveResearch(domain, researchPasses, { source: 'brief' }).catch(() => {})
    }
    ingestBriefCompany({ url, scraped, memoData }).catch(() => {})
