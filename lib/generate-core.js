...
Reference specific portfolio companies by name in at least one thesis point when portfolio is non-empty.
When portfolio is empty, cite specific mandate criteria (stage, sector, geography) from the fund thesis.
...
`
  ...
  let qualityGate = runQualityGate(memoData, fundContext, { researchMode, statMeta: statResult.statMeta })
    ...
    if (errors.length > 0) {
      try {
        ...
        if (repaired) {
          ...
          memoData = statResult.memoData
          const retryQg = runQualityGate(memoData, fundContext, { researchMode, statMeta: statResult.statMeta })
