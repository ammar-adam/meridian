import { addFund, ensureActiveFund, getAllFunds } from '@/lib/fund-profile'
 ...
 *  Seeds both Sagard + Panache, then activates a real fund so demos never land on guest "Your Fund". */
export function seedFundProfilesIfEmpty() {
  ...
  // Always leave an active fund when profiles exist (first visit or recovered empty activeId).
  const active = ensureActiveFund()
  if (active) window.dispatchEvent(new Event('meridian-context-change'))
