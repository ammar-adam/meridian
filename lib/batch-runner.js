...
function saveProcessedMemo(processed, job) {
  ...
  const id = saveMemo(processed.memoData, processed.memoId, {
...
async function runLocalBatch(job, list, {
  ...
  async function processOne(i) {
    ...
    try {
      ...
      const libraryId = saveMemo(memoData, memoId, {
        searchThesis: ctx?.thesis, companyDomain: domain,
        qualityPassed: qualityGate?.passed, sector: item.sector, stage: item.stage,
        ...buildMemoMeta({ url, searchThesis: ctx?.thesis }),
      })
      results[i] = { ...results[i], status: 'done', memoId: libraryId, companyName: memoData.COMPANY_NAME, qualityPassed: qualityGate?.passed, error: null }
    } catch (err) {
...
export async function retryBatchRow({
  ...
  try {
    ...
    const savedId = saveMemo(memoData, memoId, {
      companyDomain: domain,
      qualityPassed: qualityGate?.passed,
      ...buildMemoMeta({ url }),
    })
    results[index] = { ...results[index], status: 'done', memoId: savedId, companyName: memoData.COMPANY_NAME, qualityPassed: qualityGate?.passed, error: null }
  } catch (err) {
