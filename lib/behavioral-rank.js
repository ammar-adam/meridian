...
function buildPatternSets(trackingId = 'guest') {
  ...
  const passed = log.filter(e => e.fieldName === '_outcome' && e.newValue === 'pass')

  const pursueTokens = new Map()
  const passTokens = new Map()

  for (const e of pursued) {
    for (const t of tokenize(e.companyName)) {
      pursueTokens.set(t, (pursueTokens.get(t) || 0) + 1)
    }
  }
  for (const e of passed) {
    for (const t of tokenize(e.companyName)) {
      passTokens.set(t, (passTokens.get(t) || 0) + 1)
    }
  }

  return { pursueTokens, passTokens, pursued, passed }
}

export function scoreBehavioralFit(company, trackingId = 'guest', thesis = '') {
  const { pursueTokens, passTokens, pursued, passed } = buildPatternSets(trackingId)
