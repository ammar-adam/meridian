import { formatMetricPreferencesBlock } from '@/lib/metric-preferences'
...
Research confidence handling:
You will receive research organized by section, each tagged with a confidence
level: 'found', 'partial', or 'not_found'.

For 'found' sections: write with full confidence and specificity as normal.

For 'partial' sections: write what IS known clearly, and do not fill gaps
with plausible-sounding invented detail. If only funding stage is known but
not amount, say the stage and note the amount is undisclosed — do not guess
a number.

For 'not_found' sections: write the most honest, minimal true statement
possible. For STAT fields, write "Undisclosed" as before. For narrative
fields (like DEFENSE_1_TEXT), if genuinely no defensibility information was
found, write something honest like "Insufficient public information to
assess defensibility — recommend direct founder conversation" rather than
inventing a moat that sounds plausible. This is more useful to an analyst
than a confident-sounding fabrication.

Additionally, return CONFIDENCE_SUMMARY as a JSON array of section names
(product, funding, team, market, defensibility, news) that had 'not_found'
or 'partial' confidence in the research inputs, so the analyst can see at
a glance which parts need manual verification before forwarding.

JSON keys required (all 49 must be present):
...
THESIS_3_TITLE, THESIS_3_TEXT, CONFIDENCE_SUMMARY
...
STAT values
  ...
  Fill STAT_1, STAT_2, STAT_3 in the fund's priority order below.
  If the top-priority metric is unavailable, use the next verified signal:
  company financials → traction proxy (customers, usage, scale) → market/category context.
  Label must match value type (e.g. "Addressable market" for TAM, not "Revenue").
  If no verified number exists for a slot, use "Undisclosed" — never invent company dollars.
${formatMetricPreferencesBlock(fundContext.metricPreferences)}
...
- When preferred stats are unavailable, substitute the next verified signal tier — never invent company ARR or valuation.
...
- THESIS_1_TEXT, THESIS_2_TEXT, and THESIS_3_TEXT must EACH cite at least one real portfolio company by name with a concrete overlap (customer, distribution, pilot, or technology). All three points — not just one.
