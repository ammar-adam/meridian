import { isCanadaOnlyMandate, isCanadianMandate, looksCanadian, normalizeGeographies } from '@/lib/geography-utils'
...
export function hardFilterByGeography(companies, geographies, fundContext) {
  ...
  if (isCanadaOnlyMandate(geos, fundContext)) {
    ...
    for (const c of companies) {
      const geo = String(c?.geography || '')
      // Keep Canadian and geography-unknown; drop clearly non-Canadian.
      if (geo && !companyIsCanadian(c) && /united states|usa|latam|africa|europe|asia/i.test(geo)) {
        dropped.push(c)
      } else {
        kept.push(c)
      }
...
export function matchMandate(companies = [], { thesis = '', fundContext = null, parsed = null } = {}) {
  ...
  const stage = hardFilterByStage(geo.kept, stages)
  ...
  const outsideCoverage = matched === 0 || (geo.dropped.length > matched && coverageRatio < 0.25)
  ...
  return {
    ...
    meta: {
      ...
      coverageBanner: outsideCoverage
        ? buildCoverageBanner({ geos, thesis, matched, total, dropReason: geo.reason })
        : null,
...
function inferGeosFromThesis(thesis = '') {
