...
import { applyTrustFitPenalty, compareByTrustThenFit } from '@/lib/source-trust'
...
export function matchMandate(companies = [], { thesis = '', fundContext = null, parsed = null } = {}) {
  ...
  const geoKept = geo.kept
  const geoBannerReason = geo.reason
  ...
  const scored = stage.kept.map((c) => {
    ...
    return applyTrustFitPenalty({ ...c, fitScore: score, matchReasons })
  })

  // Dated incubator / founder rows beat AI-unverified university_scout on keyword alone.
  scored.sort(compareByTrustThenFit)
