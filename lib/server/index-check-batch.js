...
/**
 * Opportunistic: at most once per 10 minutes per warm instance;
...
export async function indexCheckIfStale({ limit = 5 } = {}) {
  ...
  // Don't hammer StartupHub from every page view.
  if (Date.now() - _lastOpportunisticAt < 10 * 60 * 1000) {
  ...
  try {
    ...
    const result = await runIndexCheckBatch({ limit, delayMs: 900 })
