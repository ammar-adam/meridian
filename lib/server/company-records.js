...
/** Companies with best sighting source — incubator/dated beats AI university_scout. */
  ...
  try {
    ...
    return await sql`
      ...
      LEFT JOIN LATERAL (
        ...
        ORDER BY
          CASE source_type
            WHEN 'incubator' THEN 0
            WHEN 'brief' THEN 1
            WHEN 'grant' THEN 2
            WHEN 'startuphub' THEN 3
            WHEN 'university_source' THEN 4
            WHEN 'scout' THEN 8
            WHEN 'university_scout' THEN 9
            ELSE 5
          END,
          CASE
            WHEN provenance ILIKE '%AI-researched%' OR provenance ILIKE '%unverified%' THEN 1
            ELSE 0
          END,
          observed_at DESC
        LIMIT 1
...
/** Stamp last_digest_at after a watch-events or digest cron pass. */
export async function touchWatchDigest(watchId) {
  const sql = sqlClient()
  if (!sql || !watchId) return false
  try {
    await ensureTables(sql)
    await sql`UPDATE mandate_watches SET last_digest_at = now(), updated_at = now() WHERE id = ${watchId}`
    return true
  } catch (e) {
    console.error('[company-records] touchWatchDigest:', e.message)
    return false
  }
}
