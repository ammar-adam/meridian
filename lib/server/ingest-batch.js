...
export async function runIngestBatch({
  ...
  for (const source of toProcess) {
    ...
    try {
      ...
      if (!ignoreHash && source.last_hash && source.last_hash === hash) {
...
/**
 * Opportunistic: seed, StartupHub bulk when below volume target, then LLM scrape.
 * Safe for request-path (pilot/benchmark) — hard-capped per call.
 */
export async function ingestIfStale({ maxAgeHours = 12, limit = 5, volumeTarget = 1500 } = {}) {
  ...
  try {
    ...
    if ((before ?? 0) < volumeTarget && isStartupHubConfigured()) {
      const hour = new Date().getUTCHours()
      const queryBatch = Math.min(12, Math.max(8, limit * 2))
      const offset = (hour * queryBatch) % STARTUPHUB_BULK_QUERIES.length
      startuphub = await runStartupHubBulk({
        offset,
        queryBatch,
        perQueryLimit: 50,
        delayMs: 200,
    ...
    if (!sources.length) {
      return {
        ran: Boolean(startuphub?.queriesRun),
        reason: startuphub?.queriesRun ? 'startuphub_only' : 'no sources after seed',
        seed,
        startuphub,
        before,
        after: await countCompanies(),
      }
    }
    ...
    let scrape = null
    if (stale.length) {
      scrape = await runIngestBatch({ cadence: 'daily', limit, ignoreHash: true })
    } else if (!startuphub?.queriesRun) {
      return { ran: false, reason: 'fresh', sourceCount: sources.length, seed, before }
    }
