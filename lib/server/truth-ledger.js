...
export async function recordAttestation({ companyName, founderName, claimerEmail, message }) {
  const sql = sqlClient()
  if (!sql || !companyName || !claimerEmail) return false
  try {
    await ensureTables(sql)
    const id = `${normalizeEntityId(companyName)}:${Date.now()}`
    await sql`INSERT INTO attestations (id, company_name, founder_name, claimer_email, message)
