...
function sqlClient() {
  ...
  if (!url) return null
  // no-store: Next.js patches global fetch and will otherwise cache the
  // Neon HTTP driver's query responses, freezing reads at first-run state.
...
async function ensureTables(sql) {
  ...
  await sql`CREATE TABLE IF NOT EXISTS attestations (
    id text PRIMARY KEY,
  ...
  // Structured claim fields — safe upgrade for pre-existing prod tables.
  await sql`ALTER TABLE attestations ADD COLUMN IF NOT EXISTS stage text`
  await sql`ALTER TABLE attestations ADD COLUMN IF NOT EXISTS raise_amount text`
  await sql`ALTER TABLE attestations ADD COLUMN IF NOT EXISTS deck_url text`
  await sql`ALTER TABLE attestations ADD COLUMN IF NOT EXISTS sectors text`
  _ensured = true
}
...
/** Prefer entities that have never been index-checked — keeps coverage growing. */
export async function listUncheckedLedgerEntities(limit = 50) {
  const sql = sqlClient()
  if (!sql) return []
  try {
    await ensureTables(sql)
    return await sql`
      SELECT e.id, e.name, e.domain, e.source, e.program, e.cohort_date, e.provenance, e.first_observed_at
      FROM ledger_entities e
      WHERE NOT EXISTS (
        SELECT 1 FROM index_checks c WHERE c.entity_id = e.id
      )
      ORDER BY e.first_observed_at ASC
      LIMIT ${limit}`
  } catch (e) {
    console.error('[truth-ledger] listUncheckedLedgerEntities:', e.message)
    return []
  }
}
...
/** Cleanup: delete synthetic rows written by external write-probes/audits. */
export async function purgeTestOutcomes() {
  const sql = sqlClient()
  if (!sql) return 0
  try {
    await ensureTables(sql)
    const rows = await sql`DELETE FROM flow_outcomes
      WHERE entity_name LIKE 'DILIGENCE_TEST%' RETURNING id`
    return rows.length
  } catch (e) {
    console.error('[truth-ledger] purgeTestOutcomes:', e.message)
    return 0
  }
}
...
/** Founder claim — stored pending; 'Founder-confirmed' only after manual review. */
export async function recordAttestation({
  companyName, founderName, claimerEmail, message,
  stage, raiseAmount, deckUrl, sectors,
}) {
  const sql = sqlClient()
  if (!sql || !companyName || !claimerEmail) return false
  try {
    await ensureTables(sql)
    const id = `${normalizeEntityId(companyName)}:${Date.now()}`
    await sql`INSERT INTO attestations
      (id, company_name, founder_name, claimer_email, message, stage, raise_amount, deck_url, sectors)
      VALUES (${id}, ${companyName}, ${founderName || null}, ${claimerEmail}, ${message || null},
              ${stage || null}, ${raiseAmount || null}, ${deckUrl || null}, ${sectors || null})`
    return true
  } catch (e) {
    console.error('[truth-ledger] recordAttestation:', e.message)
    return false
  }
}
...
export async function attestationCounts() {
  ...
  try {
    ...
    const [row] = await sql`SELECT
      ...
      FROM attestations`
    return row || { total: 0, confirmed: 0, pending: 0 }
  } catch (e) {
    console.error('[truth-ledger] attestationCounts:', e.message)
    return null
  }
}
...
export async function benchmarkStats() {
  ...
  try {
    ...
    const [tier0] = await sql`SELECT count(*)::int AS n FROM ledger_entities WHERE source = 'domain_registry'`
    return {
      ...
      preAnnouncementSignals: tier0?.n ?? 0,
    }
  } catch (e) {
