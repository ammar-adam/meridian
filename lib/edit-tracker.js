...
import { inferMetricBoostsFromEdits } from '@/lib/metric-preferences'
...
export function buildLearningContext(trackingId) {
  ...
  const inferredMetricBoosts = inferMetricBoostsFromEdits(log, trackingId)

  if (!thesisCorrections.length && !pursued.length && !passed.length && !inferredMetricBoosts.length) {
    return null
  }
  ...
  return {
    ...
    inferredMetricBoosts,
  }
}

export function logEdit({ memoId, companyName, fundName, trackingId, fieldName, originalValue, newValue, section }) {
  const log = getEditLog()
  const entry = {
    ...
    section: section || inferSection(fieldName),
...
export function logOutcome({ memoId, companyName, fundName, trackingId, outcome, editCount = 0, source, reviewerName, shareId, sector, stage }) {
  const log = getEditLog()
  const entry = {
  ...
  syncOutcomeToServer(entry)
  return entry
}

/** Fire-and-forget write to the server truth ledger (durable, cross-device). */
function syncOutcomeToServer(entry) {
  ...
  try {
    fetch('/api/outcomes', {
      ...
      body: JSON.stringify({
        ...
        outcome: entry.newValue,
        fundName: entry.fundName,
      }),
      keepalive: true,
    }).catch(() => {})
  } catch { /* offline is fine — localStorage remains the cache */ }
}
...
function inferSection(fieldName) {
  if (fieldName.startsWith('outreach_')) return 'outreach'
