import { scoreScrapeCoverage } from '@/lib/scrape-coverage'

/** Brief research depth — auto picks instant vs quick from scrape coverage. */
...
/** Pick instant for rich scrape signal; otherwise Perplexity quick. */
export function resolveEffectiveResearchMode(requestedMode, scraped) {
  const mode = resolveResearchMode(requestedMode)
  if (mode === 'auto') {
    const { level } = scoreScrapeCoverage(scraped)
    return level === 'good' ? 'instant' : 'quick'
  }
  return mode
}
