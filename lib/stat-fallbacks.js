...
export function applyStatFallbacks(memoData, {
  ...
  const signals = extractMetricSignals({ research, scraped, memoData })
  const used = new Set()
  const statMeta = []
  const next = { ...memoData }
  ...
  while (prefQueue.length < 3) {
    const extra = signals.find(s => !prefQueue.includes(s.metricId))
    if (!extra) break
  ...
  for (let i = 0; i < slots.length; i++) {
    ...
    let signal = prefId ? pickSignalForMetric(prefId, signals, used) : null

    if (!signal) {
      const extras = fillRemainingSlots(signals, used, 1)
