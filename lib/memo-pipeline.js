...
export async function prefetchResearch(url, researchMode, signal) {
  const mode = resolveResearchMode(researchMode)
  if (mode === 'instant') return buildInstantResearch(null)

  const res = await fetch('/api/research', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ url, researchMode: mode }),
    signal,
  })
  const data = await res.json()
...
/** Resolve research text — uses prefetch cache, instant body, or live fetch. */
export async function resolveResearchText(url, {
  researchMode,
  scraped,
  ...
  if (effectiveMode === 'instant') {
    return buildInstantResearch(scraped)
  }
  ...
  const res = await fetch('/api/research', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ url, researchMode: effectiveMode, forceRegenerate }),
    signal,
  })
  const data = await res.json()
...
export async function completeBriefGenerate({
  ...
  const memoId = data.cached ? `${slug}_${domain}` : `${slug}_${Date.now()}`
