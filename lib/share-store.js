...
export async function createShare({ memoData, meta = {}, userId = null }) {
  ...
  const payload = {
    ...
    meta: {
      ...
      allowOutcome: meta.allowOutcome !== false,
      memoId: meta.memoId || null,
    },
...
export async function recordShareOutcome(id, payload) {
  if (!shareEnabled()) throw new Error('Share links require DATABASE_URL')

  if (isDbEnabled()) {
    return updateShareOutcomeDb(id, payload)
  }

  const cached = await cacheGet(`share:${id}`)
  if (!cached) return null
  cached.meta = {
    ...cached.meta,
    outcome: payload.outcome,
    reviewerName: payload.reviewerName || null,
    outcomeNote: payload.note || null,
    outcomeAt: new Date().toISOString(),
  }
  await cacheSet(`share:${id}`, cached, SHARE_TTL_MS)
  return cached
}
