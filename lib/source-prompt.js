...
- source: "pitchbook" | "perplexity" | "both"
- domain: company website domain without protocol
- url: full https URL (infer from domain if needed)
...
export function buildRankUserMessage(thesis, parsed, pitchbookResults, perplexityResearch, fundContext = null) {
  const blocks = buildRankUserBlocks(thesis, parsed, pitchbookResults, perplexityResearch, fundContext)
  return blocks.map(b => b.text).join('\n')
}

/** Block array for Claude prompt caching — fund context cached, research dynamic */
export function buildRankUserBlocks(thesis, parsed, pitchbookResults, perplexityResearch, fundContext = null) {
...
PitchBook results (${pitchbookResults.length} companies):
${JSON.stringify(pitchbookResults.slice(0, 40), null, 2)}
`
