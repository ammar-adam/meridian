...
export async function runResearch(url, {
  ...
  if (mode === 'instant') {
    ...
    return { research: buildInstantResearch(scraped), cached: false, instant: true }
  }
  ...
  const query = quick
    ? `Research ${url}. Return concise bullets: product, TAM estimate, funding (stage/amount/lead investors), founders, 2 defensibility points, recent news. Use numbers only when sourced.`
    : `
