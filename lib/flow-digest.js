...
export function buildFlowDigest({
  ...
  const verifiedMiss = list.filter(c =>
    c.coverage?.status === 'community_first'
    || c.ledger?.verification?.status === 'verified_miss'
  )
  // Honest reach: verified email or direct LinkedIn only (matches reachabilitySummary.rate)
  ...
  return {
    ...
    stats: {
      ...
      communityFirst: verifiedMiss.length,
      communitySourced: community.length,
