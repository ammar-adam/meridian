...
/** Fire-and-forget sync to the server-side watch store (Slice A). */
function syncWatchToServer(watch) {
  if (typeof window === 'undefined' || !watch) return
  try {
    fetch('/api/watches', {
      method: 'POST',
  ...
  } catch { /* offline is fine — localStorage remains source of truth */ }
}

function syncWatchRemovalToServer(fundId) {
  if (typeof window === 'undefined' || !fundId) return
  try {
    fetch(`/api/watches?fundId=${encodeURIComponent(fundId)}`, { method: 'DELETE' }).catch(() => {})
  } catch { /* ignore */ }
}
