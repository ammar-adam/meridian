...
export async function getWorkspace(userId) {
  ...
  return {
    ...
    teamContext: ws?.teamContext ?? null,
...
export async function saveWorkspace(userId, { fundsStore, memos: memoList, edits: editList, teamContext }) {
  const db = getDb()
  if (!db) return false

  const values = { userId, fundsStore, updatedAt: new Date() }
  const sets = { fundsStore, updatedAt: new Date() }
  if (teamContext !== undefined) {
    values.teamContext = teamContext
    sets.teamContext = teamContext
  }
...
export async function isTeamMemberDb(userId, teamId) {
