...
/** If funds exist but none is active, prefer Panache for Canadian demos, else first fund. */
export function ensureActiveFund() {
  ...
  const preferred = store.funds.find(f => f.id === 'panache_ventures') || store.funds[0]
  store.activeFundId = preferred.id
  writeStore(store)
  return getFundProfile()
}
