...
export function getFundProfile(fundId) {
  ...
  const id = fundId || store.activeFundId || store.funds[0].id
...
export function addFund(draft) {
  const fund = createFundProfile(draft)
  let store = readStore()
  if (!store) store = { activeFundId: fund.id, funds: [] }
  if (!store.funds.some(f => f.id === fund.id)) store.funds.push(fund)
  store.activeFundId = fund.id
  writeStore(store)
  return fund
}
...
export function hasFundProfile() {
  ...
  return funds.some(f => {
    ...
    return f.fundName && s?.thesis?.trim()
  })
}
