...
import { hasClaimedUserFund, markUserFundClaimed } from '@/lib/investor-identity'
...
export function createFundProfile(draft = {}) {
  ...
  return {
    ...
    investorType: draft.investorType || null,
    portfolio: normalizePortfolio(draft.portfolio),
    strategies,
...
/** True when the visitor finished Sign in / welcome and has a usable mandate. */
export function hasUserFundProfile() {
  return hasClaimedUserFund() && hasFundProfile()
}

export function clearFundsStore() {
  if (typeof localStorage === 'undefined') return
  localStorage.setItem(FUNDS_STORE_KEY, JSON.stringify({ activeFundId: null, funds: [] }))
  if (typeof window !== 'undefined') {
    window.dispatchEvent(new Event('meridian-context-change'))
  }
}

/** Persist a user-claimed firm and mark identity complete. */
export function claimUserFund(draft) {
  const profile = saveFundProfile(createFundProfile(draft))
  markUserFundClaimed()
  return profile
}

/** If funds exist but none is active, activate the first fund (no Panache preference). */
