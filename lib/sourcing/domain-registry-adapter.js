...
const NON_STARTUP_NAME = /\b(foundation|church|parish|synagogue|mosque|temple|society|association|club|union|trust|estate of|ministry|congregation)\b/i
...
export async function runDomainRegistryAdapter({
  ...
  const entities = withDomains
    ...
    .map(c => ({
      ...
      confidence: 'low',
      provenance: `Incorporated ${c.incorporationDate || 'recently'}, domain ${c.domain} live`,
      sourceMeta: {
