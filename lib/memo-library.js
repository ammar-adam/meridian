...
export function saveMemo(memoData, id, meta = {}) {
  ...
  const fundId = meta.fundId ?? existing?.fundId ?? null
  ...
  const withoutDupes = domain && fundId
    ? library.filter(e => e.id === entryId || !(e.companyDomain === domain && (e.fundId || 'guest') === (fundId || 'guest')))
