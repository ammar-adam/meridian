...
function cacheFile() {
  // Serverless filesystems are read-only outside /tmp — fall back there.
  const candidates = [
    path.join(process.cwd(), '.cache', 'domain-check'),
    path.join(os.tmpdir(), 'meridian-cache', 'domain-check'),
  ]
  ...
  return path.join(os.tmpdir(), 'dns-results.json')
}
