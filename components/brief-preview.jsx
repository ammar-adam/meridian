      ...
      {scraped.ogImage ? (
        <div className="relative h-28 w-full bg-zinc-100">
          ...
          {loading && (
            <div className="absolute inset-0 flex items-center justify-center bg-black/30">
              <span className="rounded-full bg-white/90 px-3 py-1 text-[11px] font-medium">Researching…</span>
            </div>
          )}
        </div>
      ) : (
        <div className="flex h-20 items-center justify-center bg-zinc-100">
          {scraped.favicon ? (
            ...
            <span className="flex h-12 w-12 items-center justify-center rounded-xl bg-zinc-900 text-lg font-semibold text-white">{initial}</span>
          )}
        </div>
      )}
